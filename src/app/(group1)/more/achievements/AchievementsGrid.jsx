            ...
            <div className="container">
                <div className="lg:pl-11">
                    ...
                    <h2 className="text-2xl font-bold text-primary-foreground mb-5">
                        {filteredAchievements.length} {filter === 'All' ? '' : filter} {" "} Achievements {filteredAchievements.length > 1 ? 'Activities' : 'Activity'}
                    </h2>

                    <div className="grid md:grid-cols-3 grid-cols-1 gap-y-7.5 lg:gap-x-[45px] gap-x-5 lg:pt-15 pt-10">
                        {filteredAchievements.map(({id, title, description, date, image, category}) => (
                            <CardTwo
                                key={id}
                                id={id}
                                src={image}
